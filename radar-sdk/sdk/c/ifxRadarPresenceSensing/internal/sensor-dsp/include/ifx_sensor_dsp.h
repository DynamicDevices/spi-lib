 ...
 * \{
 *
 * The Sensor-DSP library provides signal processing functions required to
 * support the implementation of different sensor applications, i.e. radar
 * gesture recognition, vital sensing and presence detection.
 *
 * <b>Features:</b>
 ...
 * the Sensor-DSP library is included in your project as ModusToolbox
 * dependency.
 *
 * 3. Include Sensor-DSP in the main.c file:
 * \snippet snippet/main.c snippet_required_includes
 *
 * 4. The following code shows a simple example calculating the Doppler FFT on
 * radar data frame: \snippet snippet/main.c snippet_sensor_dsp
 */
...
#ifdef _WIN32
#ifdef IFX_SENSOR_DSP_EXPORT
#define IFX_SENSOR_DSP_EXPORTED __declspec(dllexport)
#else
#define IFX_SENSOR_DSP_EXPORTED __declspec(dllimport)
#endif
#else
#define IFX_SENSOR_DSP_EXPORTED
#endif
...
typedef _Fcomplex cfloat32_t;

#else
/** Complex float number type */
typedef float _Complex cfloat32_t;

// Windows SDK lacks proper C complex support
#define _FCbuild(re, im) \
    (re + (I * im))

#endif

static inline cfloat32_t ifx_add_f32(cfloat32_t a, cfloat32_t b) {
    #if !defined(_WIN32)
    return a + b;
    #else
...
static inline cfloat32_t ifx_sub_f32(cfloat32_t a, cfloat32_t b) {
    #if !defined(_WIN32)
    return a - b;
    #else
    return _FCbuild(crealf(a) - crealf(b), cimagf(a) - cimagf(b));
...
static inline cfloat32_t ifx_div_real_f32(cfloat32_t a, float32_t b) {
    #if !defined(_WIN32)
    return a / b;
    #else
    return _FCbuild(crealf(a) / b, cimagf(a) / b);
    #endif
}

static inline cfloat32_t ifx_mul_real_f32(cfloat32_t a, float32_t b) {
    #if !defined(_WIN32)
    return a * b;
    #else
    return _FCbuild(crealf(a) * b, cimagf(a) * b);
...
typedef struct {
    ...
    float32_t threshold; /**< Required threshold of peaks, the vertical distance
                            to its neighboring samples */
    int32_t distance;    /**< Required minimal horizontal distance (>= 1) in
...
/**
 * @brief Substracts the calculated array mean from each element of the floating
 * point array.
 *
...
/**
 * @brief Substracts the calculated array mean from each element of the complex
 ...
 * @return none
 */
 void ifx_cmplx_mean_removal_f32(cfloat32_t *v,
                                                        uint32_t len);

/**
 * @brief Calculate range FFT from real floating point raw radar data.
 * Perform optional mean removal and windowing on the raw radar data prior to 1D
 * FFT. The caller must allocate the memory for the frame and range arrays.
 *
 * @param[inout] frame Pointer to raw radar real data of shape
 * [num_chirps_per_frame][num_adc_samples]
 * @note frame is modified by this function if mean_removal is true and/or win
 * != NULL
 ...
 *         - \ref IFX_SENSOR_DSP_ARGUMENT_ERROR : Not supported FFT length
 * (num_samples_per_chirp)
 */
int32_t
ifx_range_fft_f32(float32_t *frame, cfloat32_t *range, bool mean_removal,
                  const float32_t *win, uint16_t num_samples_per_chirp,
 ...
 * [num_chirps_per_frame][num_adc_samples]
 * @note Processing by this function occurs in-place. The raw radar complex data
 * is replaced by the calculated range FFT
 * @param[in] mean_removal If true, remove mean along samples before 1D FFT
 * @param[in] win Pointer to window to be applied to the raw radar data prior 1D
 * FFT
 * @note Can be NULL if not windowing is desired
 * @param[in] num_samples_per_chirp Number of samples per radar chirp
 * @param[in] num_chirps_per_frame Number of chirps per radar frame
 ...
 * (num_samples_per_chirp)
 */
int32_t ifx_range_cfft_f32(
    cfloat32_t *frame, bool mean_removal, const float32_t *win,
    uint16_t num_samples_per_chirp, uint16_t num_chirps_per_frame);

/**
 * @brief Calculate doppler FFT from range data.
 * Perform optional mean removal and windowing on the range data prior to 1D
 * FFT. The caller must allocate the memory for the range and doppler arrays.
 *
 * @param[inout] range Pointer to range complex data of shape
 * in [num_chirps_per_frame][num_range_bins]
 * out [num_doppler_bins][num_range_bins]
 * @param[out] doppler Pointer to transformed range doppler complex data of
 * shape [num_range_bins][num_doppler_bins]
 * @param[in] mean_removal If true, remove mean along samples before 1D FFT
 * @param[in] win Pointer to window to be applied to the range data prior 1D FFT
 * @note Can be NULL if not windowing is desired
 * @param[in] shift_fft Shift zero-frequency component
 ...
 */
int32_t
ifx_doppler_cfft_f32(cfloat32_t *range, cfloat32_t *doppler, bool mean_removal,
                     bool shift_fft, const float32_t *win,
                     uint16_t num_range_bins, uint16_t num_chirps_per_frame);
 ...
 * using the following formula
   ...
   0.08 \cos\left( \frac{4\pi n}{N-1} \right),\qquad \mathrm{0} \le\ n <
 \mathrm{N} \f]
 * The values are populated in the output array 'window' whose pointer is passed
 as a parameter.
 * The caller is responsible to allocate memory for output. For
 * a window of length \f$ N \f$ a memory of \f$ N*sizeof(float32_t) \f$ should
 be allocated.
 *
 * @param[inout] win Starting pointer to float32_t array to be populated with
 output
 * @param[in] len Length of window to be generated
 * @return None
 */
void ifx_window_blackman_f32(float32_t *win,
                                                     uint32_t len);

/**
 * @brief Generate a symmetric four-term Blackman-Harris window.
 *
 * The function generates a symmetric four-term Blackman-Harris window \f$w\f$
 of length \f$N\f$
 * using the following formula
   \f[
   w_n = \mathrm{a_0} - \mathrm{a_1} \cos\left( \frac{2\pi n}{N-1} \right) +
   \mathrm{a_2} \cos\left( \frac{4\pi n}{N-1} \right) -
   \mathrm{a_3} \cos\left( \frac{6\pi n}{N-1} \right),\qquad \mathrm{0} \le\ n <
 \mathrm{N} \f]
 *
 ...
 *
 * The window values are populated in the output array 'window' whose pointer is
 passed as a
 * parameter.
 * The caller is responsible to allocate memory for output. For
 * a window of length \f$ N \f$ a memory of \f$ N*sizeof(float32_t) \f$ should
 ...
 * @return None
 */
void ifx_window_blackmanharris_f32(float32_t *win,
                                                           uint32_t len);
 ...
 * @return None
 */
void ifx_window_hamming_f32(float32_t *win,
                                                    uint32_t len);
...
/**
 * @brief Generate a steering vector for AOA estimation given the theta range,
 * number of beams, and number of antennas Defines a method for generating
 * steering vector data input The generated steering vector will span from
 * -angEstRange to angEstRange with increments of ang_est_resolution The
 * generated steering vector is already transposed and can be used directly for
 * all further AOA estimations (bartlett/capon)
 *
 * @param[in] ang_est_range Single side angle of view in radians. zero is
 * directly perpendicular to antenna plane
 ...
 * @param[inout] pDst Pointer to output steering matrix array allocated by the
 * user [ rows, columns ] = [ num_angles, num_antennas ]
 * @return none
 */
void ifx_gen_steering_matrix_f32(
    float32_t ang_est_range, uint32_t num_angles, float32_t antenna_spacing_mm,
    float32_t lambda_mm, uint32_t num_ant, const arm_matrix_instance_f32 *pDst);

/**
 * @brief Generate a phase shift beam forming matrix
 * Defines a method for generating a phase shift beam forming matrix by
 * multiplying the steering matrix by the input.
 *
 ...
 * @return status flag
 */
arm_status ifx_angle_dbf_f32(
    const arm_matrix_instance_f32 *pInput,
    const arm_matrix_instance_f32 *pSteering, arm_matrix_instance_f32 *pOutput);
 ...
 * @return None
 */
void ifx_shift_cfft_f32(cfloat32_t *v, uint32_t len,
                                                uint32_t dim);
 ...
 * @param[in] alpha Exponential moving average parameter (0 highest historical
 * influence, 1 lowest)
 ...
 * @param[out] out_data Pointer to output data with movement indication (size
 * must be identical as in_data)
 */
 ...
 *
 * The function searches for peaks in an array. A peak is a point that is larger
 * than its left and its right neighbor point. The pointer array together with
 * the length len denote the left and right end of the search range.
 *
 * The function searches for the max_peaks highest peaks and returns the number
 * of peaks found. The function may find 0 peaks up to max_peaks peaks. The
 * indices of the peaks are written to the array indices.
 *
 * @param [in] x             Pointer to array for peak search
 * @param [in] length        Number of elements in array
 * @param [out] peak_indices Pointer to array that holds the indices of the
 * peaks found. The array must hold at least max_peaks elements.
 * @param [in] max_peaks Maximum number of peaks
 * @param [in] opts      Pointer to conditions structure for peaks
 * @retval               Number of peaks found. The corresponding indices are
 * written to peaks
 */
int32_t
ifx_peak_search_f32(const float32_t *x, int32_t length, int32_t *peak_indices,
                    int32_t max_peaks, const ifx_peak_search_opts_f32_t *opts);
 ...
 * @return none
 */
 void ifx_rotate_f32(float32_t *v, uint32_t len,
                                            uint32_t k);
...
/**
 * @brief Calculate range resolution including number of samples, sample rate
 * and frequency slope
 *
 ...
 * @return Range resolution
 */
static inline float32_t ifx_range_resolution_ex(uint32_t num_samples,
                                                float32_t sample_rate_hz,
                                                float32_t freq_slope_hz_s) {
    const float32_t bandwidth =
        (freq_slope_hz_s / sample_rate_hz) * (float32_t)num_samples;
 ...
 * @return Max range
 */
static inline float32_t ifx_max_range(uint32_t sample_rate_hz,
                                      float32_t freq_slope_hz_s) {
    return (((float32_t)sample_rate_hz * IFX_LIGHT_SPEED_M_S) /
            (2.0F * freq_slope_hz_s));
}
 ...
 */
static inline float32_t ifx_doppler_resolution(float32_t bandwidth_hz,
                                               float32_t frame_time_s) {
 ...
 * @return Max Doppler
 */
static inline float32_t ifx_max_doppler(float32_t bandwidth_hz,
                                        float32_t chirp_time_s) {
 ...
 *
 * @param[in] phase_a First vector of phase
 * @param[in] phase_b: Second vector of phase
 * @param[in] size Length of the phase vector
 ...
 *                       ARM_MATH_ARGUMENT_ERROR on error
 */
arm_status ifx_angle_monopulse_f32(
    const float32_t *phase_a, const float32_t *phase_b, uint32_t size,
    float32_t wavelength, float32_t antenna_spacing, float32_t *angle);
